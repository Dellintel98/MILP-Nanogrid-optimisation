...
class Nanogrid:
    ...
    def create_parameters(self):
        ...
        self.model.P_ev_max = pe.Param(self.model.time_set, self.model.parking_lot_set,
                                       initialize=parking_lots['max_charging'], default=0)
    ...
    def set_costs_variables(self):
        ...
        self.model.C_ee_daily = pe.Var(domain=pe.NonNegativeReals)
        self.model.C_ee_hourly = pe.Var(self.model.time_set, domain=pe.NonNegativeReals)
        self.model.C_profit_daily = pe.Var(domain=pe.NonNegativeReals)
        ...
        self.model.C_variations_daily = pe.Var(domain=pe.NonNegativeReals)
    ...
    def create_objective(self):
        ...
        def variations_daily(model):
            return model.C_variations_daily == (
        ...
        self.model.variations_daily = pe.Constraint(rule=variations_daily)

        def variations_penalization(model):
            return model.C_variations == model.C_variations_daily

        self.model.variations_penalization = pe.Constraint(rule=variations_penalization)

        def total_costs(model):
            return model.C_total == model.C_ee_operational - model.C_profit
    ...
    def create_operational_costs_constraints(self):
        def total_operational_electricity_costs(model):
            return model.C_ee_operational == model.C_ee_daily

        self.model.total_operational_electricity_costs = pe.Constraint(rule=total_operational_electricity_costs)

        def daily_electricity_costs(model):
            return model.C_ee_daily == sum(model.C_ee_hourly[t] for t in model.time_set)

        self.model.daily_electricity_costs = pe.Constraint(rule=daily_electricity_costs)
    ...
    def create_profit_constraints(self):
        def total_profit(model):
            return model.C_profit == model.C_profit_daily

        self.model.total_profit = pe.Constraint(rule=total_profit)

        def daily_profit(model):
            return model.C_profit_daily == sum(model.C_profit_hourly[t] for t in model.time_set)

        self.model.daily_profit = pe.Constraint(rule=daily_profit)
    ...
    def create_power_balance_equation(self):
        ...
        def power_balance_equation(model, t):
            P_pv_t = model.P_pv_install * model.pv_system_data[t]

            return model.P_grid[t] + P_pv_t + model.P_battery_ds[t] + \
                   sum(model.P_ev_ds[lot, t] for lot in model.parking_lot_set) * ev_ds_condition \
                   == \
                   sum(model.P_ev_ch[lot, t] for lot in model.parking_lot_set) + \
                   model.P_battery_ch[t] + model.P_OD_data[t] * building_inclusion_condition
    ...
    def create_grid_constraints(self):
        ...
        def negative_grid_power(model, t):
            P_pv_t = model.P_pv_install * model.pv_system_data[t]
            return model.P_grid_negative[t] <= P_pv_t
    ...
    def create_battery_system_constraints(self):
        ...
        def constant_voltage_mode_battery_power(model, t):
            return model.P_battery_max[t] <= self.dt * ((model.E_battery_capacity - model.E_battery[t]) /
    ...
    def create_electric_vehicle_constraints(self):
        ...
        def constant_current_mode_ev_charging_power(model, lot, t):
            if model.PL_ev_available[t, lot] == 1:
                return model.P_ev_ch[lot, t] <= model.P_ev_max[lot, t]
            else:
                return pe.Constraint.Skip
        ...
        def constant_voltage_mode_ev_charging_power(model, lot, t):
            if model.PL_ev_available[t, lot] == 1:
                return model.P_ev_ch[lot, t] <= model.P_ev_max[lot, t] * (1 - model.SOE_ev_relative[lot, t]) \
                       / (1 - model.components_data["BS_CC_CV_switch"])
            else:
        ...
        if self.model_variant == 2 or self.model_variant == 4:
            def ev_discharging_power(model, lot, t):
                if model.PL_ev_available[t, lot] == 1:
                    return model.P_ev_ds[lot, t] <= model.P_ev_max[lot, t]
                else:
    ...
    def save_optimisation_results_to_excel(self):
        ...
        main_directory = '\\evcs_optimisation_results\\'
        ...
        file_subdirectory = f'model_{self.model_variant}\\'
